...
class Infer():
    ...
    def Predict(self, img_path, class_list, vis_threshold=0.4, output_folder='Inference'):
        ...
        t0 = time.time()
        with torch.no_grad():
            scores, labels, boxes = self.system_dict["local"]["model"](
                img.cuda().permute(2, 0, 1).float().unsqueeze(dim=0))
            boxes /= scale;
        ...
        try:
            if boxes.shape[0] > 0:
                ...
                for box_id in range(boxes.shape[0]):
                    ...
                    text_size = \
                    cv2.getTextSize(class_list[pred_label] + ' : %.2f' % pred_prob, cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
