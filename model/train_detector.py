...
class Detector():
    ...
    def Train_Dataset(self, root_dir, coco_dir, img_dir, set_dir, batch_size=8, image_size=512, use_gpu=False,
                      num_workers=3):
        ...
        self.system_dict["local"]["training_params"] = {
            "batch_size": self.system_dict["params"]["batch_size"], # * self.system_dict["local"]["num_gpus"],
            "shuffle": True,
            "drop_last": True,
            "collate_fn": collater,
        ...
        self.system_dict["local"]["training_set"] = CocoDataset(
            root_dir=self.system_dict["dataset"]["train"]["root_dir"] + "/" + self.system_dict["dataset"]["train"][
                "coco_dir"],
    ...
    def Val_Dataset(self, root_dir, coco_dir, img_dir, set_dir):
        ...
        self.system_dict["local"]["val_set"] = CocoDataset(
            root_dir=self.system_dict["dataset"]["val"]["root_dir"] + "/" + self.system_dict["dataset"]["val"][
                "coco_dir"],
    ...
    def Set_Hyperparams(self, lr=0.0001, val_interval=1, es_min_delta=0.0, es_patience=0):
        ...
        self.system_dict["local"]["scheduler"] = torch.optim.lr_scheduler.ReduceLROnPlateau(
    ...
    def Train(self, num_epochs=2, model_output_dir="trained/"):
        ...
        if (self.system_dict["dataset"]["val"]["status"]):
                    ...
                    try:
                        ...
                        if torch.cuda.is_available():
                            cls_loss, reg_loss = self.system_dict["local"]["model"](
                                [data['img'].to(self.system_dict["local"]["device"]).float(),
                                 data['annot'].to(self.system_dict["local"]["device"])])
                        else:
                            cls_loss, reg_loss = self.system_dict["local"]["model"](
                        ...
                        print(
                            'Epoch: {}/{}. Iteration: {}/{}. Cls loss: {:.5f}. Reg loss: {:.5f}. Batch loss: {:.5f} Total loss: {:.5f}'.format(
                                epoch + 1, self.system_dict["params"]["num_epochs"], iter + 1, num_iter_per_epoch,
                ...
                if epoch % self.system_dict["params"]["val_interval"] == 0:
                    ...
                    if loss + self.system_dict["params"]["es_min_delta"] < self.system_dict["output"]["best_loss"]:
                        ...
                        torch.save(self.system_dict["local"]["model"],
                                   os.path.join(self.system_dict["output"]["saved_path"],
                        ...
                        if isinstance(self.system_dict["local"]["model"], nn.DataParallel):
                            self.system_dict["local"]["model"].module.backbone_net.model.set_swish(
            ...
            for epoch in range(self.system_dict["params"]["num_epochs"]):
                ...
                epoch_loss = []
                print(self.system_dict["local"]["training_generator"])
                progress_bar = tqdm(self.system_dict["local"]["training_generator"])
                for iter, data in enumerate(progress_bar):
                    print(iter)
                    #if iter == 15:
